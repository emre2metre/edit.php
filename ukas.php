	...
	*/
	...
	function ukas_giris($ukas_ayar, $ukas_yonlendir, $ukas_uyariyazisi, $ukas_hatayazisi) { 
	    ...
	    if (isset($_POST["giris"])) { // Giriş yapılmışsa
	        ...
	        $sifre =
		htmlspecialchars(trim(str_replace(" ", "", $_POST["sifre"]))); // Şifre
			...
			if ($_POST["_token"] !== $_SESSION["_token"]) { die('Hak yeme hack yeme!'); }*/
					...
					$_SESSION["uye_telno"] 			= $fetch["uye_telno"];
					$_SESSION["uye_evadres"] 		= $fetch["uye_evadres"];
							...
							if ($sifre == $sifret) { // Şifre ile şifre tekrar uyuşuyorsa
								...
								if ($kayit) { // Kayıt başarılıysa
						            ...
						            if ($rowcount) { // Kayıt yaptıktan sonra giriş yap
										...
										$_SESSION["uye_telno"] 			= $fetch["uye_telno"];
										$_SESSION["uye_evadres"] 		= $fetch["uye_evadres"]; // üye onayı
	...
	function ukas_profil($p){
		...
		global $ukas_profil_telno;
		global $ukas_profil_evadres;
		...
		if($uyesaydirma > 0){ // üye varsa
			...
			// gerekli verileri değişkenlere depolayalım
		        ...
		        $ukas_profil_telno 	= $uye_cek["uye_telno"];
				$ukas_profil_evadres 	= $uye_cek["uye_evadres"];
		}
	}
